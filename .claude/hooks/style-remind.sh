...
case "$fp" in
  */src/*.rs | src/*.rs)
    printf '%s' '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"VeriFlat Verus style — learn it for THIS section BEFORE writing. Match .kiro/steering/verus-style.md and the canonical files (syscall_alloc_quota_4k + the locker_unlocker.rs wrappers); open the nearest LIVE sibling and copy its shape. Tells: bare requires (no // comments); single-line // ---- ---- banners ONLY in ensures and inv() re-establishment; comment-free proof {} blocks; #![auto] on shallow framing foralls, hand #![trigger] on deep ones, NEVER #![all_triggers]; spec files hold only specs; #[verifier::spinoff_prover] on new wrappers/lemmas."}}'
    ;;
esac
exit 0
