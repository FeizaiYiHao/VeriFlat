...
impl PageTableDom {
    ...
    pub fn map_4k_page(&mut self, pagetable_root: RwLockPageTableRoot, Tracked(lock_manager): Tracked<&LockManager>, Tracked(lock_perm): Tracked<&LockPerm>, 
        target_l4i: L4Index,
        target_l3i: L3Index,
        target_l2i: L2Index,
        target_l1i: L2Index,
        ...
        requires
            old(self).inv(),
            old(self).dom().contains(pagetable_root),
            old(self)[pagetable_root].wlocked_by(lock_manager) == true,
            old(self)[pagetable_root].inv(),
        ...
        ensures
            ...
            self[pagetable_root].wlocked_by(lock_manager) == true,
    ...
    {
        let mut pagetable = self.map.take(pagetable_root, Tracked(lock_manager), Tracked(lock_perm));
        pagetable.map_4k_page(target_l4i, target_l3i, target_l2i, target_l1i, target_l1_p, target_entry);
        self.map.put(pagetable_root, Tracked(lock_manager), Tracked(lock_perm), pagetable);
    }
}
