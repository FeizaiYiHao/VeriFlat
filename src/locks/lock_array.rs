...
verus! {
    ...
    impl<T:LockInvTrait + LockMajorTrait + LockOwnerIdTrait, ROT, KGhostT, UGhostT,
        ...
        /// Exact alignment between this array and one typed held-lock family.
        #[verifier::opaque]
        pub open spec fn typed_lock_map_aligned(
            ...
            &&& (forall|index: usize|
                ...
                #![trigger self.spec_index(index).view()
                    .locked_by_thread(thread_id)]
                held_locks.dom().contains(index) == {
                    &&& index_valid(N, index)
                    &&& self.spec_index(index).view()
                        .locked_by_thread(thread_id)
                }
                && (held_locks.dom().contains(index) ==>
                    held_locks.index(index).lock_id
                        == self.spec_index(index).lock_id()))
            &&& (forall|index: usize|
                #![trigger typed_lock_map_contains_mode(
                    held_locks, index, TypedLockMode::Read)]
                #![trigger self.spec_index(index).view()
            ...
            &&& (forall|index: usize|
                #![trigger typed_lock_map_contains_mode(
                    held_locks, index, TypedLockMode::Write)]
                #![trigger self.spec_index(index).view()
        ...
        pub fn borrow_mut_typed<'a>(
            ...
            ensures
                ...
                final(self).spec_index(index).view().wlocked_by(lctx),
                final(self).spec_index(index).view().write_lock_perm_match(lp.view()),
                final(self).spec_index(index).view().view_rodata()
                    == old(self).spec_index(index).view().view_rodata(),
                final(self).spec_index(index).view().view_kernel_ghost()
                    == old(self).spec_index(index).view().view_kernel_ghost(),
                final(self).spec_index(index).view().view_user_ghost()
                    == old(self).spec_index(index).view().view_user_ghost(),
                final(self).spec_index(index).view().locking_thread()
                    == old(self).spec_index(index).view().locking_thread(),
                final(self).spec_index(index).view().being_killed()
                    == old(self).spec_index(index).view().being_killed(),
                *ret == old(self).spec_index(index).view().view(),
                final(self).spec_index(index).view().view() == *final(ret),
                final(self).spec_index(index).lock_id()
                    == old(self).spec_index(index).lock_id()
                    ==> final(self).typed_lock_map_aligned(
                        held_locks, lctx.thread_id()),
        {
            proof {
                // Unfold before the returned `&mut T` starts borrowing `self`;
                // a post-borrow proof cannot legally inspect `final(self)`.
