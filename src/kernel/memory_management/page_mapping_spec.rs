...
verus! {
    ...
    pub open spec fn mapped_4k_page_pagetable_wf_inner(pagetable_map: LockedMap<RwLockPageTableRoot, PageTable<PT_TYPE>, (), PAGE_TABLE_HAS_KILL_STATE>, page_array: LockedArray<Page, (), NUM_PAGES, NO_KILL_STATE>) -> bool {
        &&&
        forall|p_i:PageIndex, pt_ptr:RwLockPageTableRoot, va: VAddr|
            ...
            &&
            page_array.spec_index(p_i).view().view().state is Mapped4k
            &&
        ...
        forall|pt_ptr:RwLockPageTableRoot, va: VAddr|
            ...
            pagetable_map.spec_index(pt_ptr).view().mapping_4k().contains_key(va)
            ==>
            page_array.spec_index(page_ptr2page_index(pagetable_map.spec_index(pt_ptr).view().mapping_4k()[va].addr)).view().view().state is Mapped4k
            &&
