...
impl Kernel{
    ...
    pub open spec fn cpu_pagetable_pointers_wf(&self) -> bool{
        ...
        {
            &&&
            self.get_pagetable_dom().contains(self.get_cpu(cpu_id).view().current_pagetable)
            &&&
            {
                |||
    ...
    pub open spec fn cpu_dirty_map_contains_only_alive_pagetable(&self) -> bool{
        ...
        ==>
        self.get_pagetable_dom().contains(self.get_cpu(cpu_id).view().tlb_dirty_bitmap()[pcid].unwrap())
        &&
