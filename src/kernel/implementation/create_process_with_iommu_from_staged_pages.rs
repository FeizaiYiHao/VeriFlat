...
pub fn create_process_with_iommu_from_staged_pages(
    ...
    Tracked(iommu_table_page_lock_perm): Tracked<&LockPerm>,
    Tracked(iommu_l4_page_lock_perm): Tracked<&LockPerm>,
    ...
    ensures
        final(krnl).inv(),
        kernel_u_create_process_with_iommu_changed(kernel_k_to_kernel_u(*old(krnl)), kernel_k_to_kernel_u(*final(krnl)), parent_ptr, process_page_ptr),
        ...
        final(krnl).it_mp.spec_index(iommu_table_page_ptr).wlocked_by(final(lctx)),
        ...
        final(krnl).it_mp.spec_index(iommu_table_page_ptr).view().proc_ptr == process_page_ptr,
        final(krnl).it_mp.spec_index(iommu_table_page_ptr).view().pcid is None,
        final(krnl).it_mp.spec_index(iommu_table_page_ptr).view().kernel_l4_end == 0,
        ...
        final(krnl).thr_mp.spec_index(staging_thread_ptr).view().quota_4k == old(krnl).thr_mp.spec_index(staging_thread_ptr).view().quota_4k - 5,
        ...
        final(krnl).pg_arr.spec_index(page_ptr2page_index(iommu_table_page_ptr)).view().wlocked_by(final(lctx)),
        final(krnl).pg_arr.spec_index(page_ptr2page_index(iommu_l4_page_ptr)).view().wlocked_by(final(lctx)),
        process_page_lock_perm.state() is WriteLock,
        ...
        final(lctx).iommu_table_lock_map() == old(lctx).iommu_table_lock_map().insert(iommu_table_page_ptr, TypedHeldLock { lock_id: final(krnl).it_mp.lock_id_by_key(iommu_table_page_ptr), mode: TypedLockMode::Write }),
        ...
        final(lctx).page_lock_map() == old(lctx).page_lock_map()
            .insert(page_ptr2page_index(iommu_l4_page_ptr), TypedHeldLock { lock_id: final(krnl).pg_arr.lock_id_by_index(page_ptr2page_index(iommu_l4_page_ptr)), mode: TypedLockMode::Write })
            .insert(page_ptr2page_index(iommu_table_page_ptr), TypedHeldLock { lock_id: final(krnl).pg_arr.lock_id_by_index(page_ptr2page_index(iommu_table_page_ptr)), mode: TypedLockMode::Write })
        ...
        lock_id_set_aligned(final(lctx)),
{
    ...
    let Tracked(iommu_l4_page_perm) = take_perm_4k(iommu_l4_page_mut);
    iommu_l4_page_mut.state = PageState::IOMMUTable { iommu_table_root: iommu_table_page_ptr };
    proof { lctx.update_lock_id(KernelObjId::Page(iommu_l4_page_index), old_iommu_l4_page_lock_id, krnl.pg_arr.lock_id_by_index(iommu_l4_page_index)); }
    ...
    let Tracked(iommu_table_page_perm) = take_perm_4k(iommu_table_page_mut);
    iommu_table_page_mut.state = PageState::Allocated4k { state: Allocated4KPageState::AsIommuTableRoot };
    proof { lctx.update_lock_id(KernelObjId::Page(iommu_table_page_index), old_iommu_table_page_lock_id, krnl.pg_arr.lock_id_by_index(iommu_table_page_index)); }
    ...
    staging_thread_mut.temp_alloc_cache_4k = Ghost(staging_thread_mut.temp_alloc_cache_4k.view().remove(process_page_ptr).remove(pagetable_page_ptr).remove(l4_page_ptr).remove(iommu_table_page_ptr).remove(iommu_l4_page_ptr));
