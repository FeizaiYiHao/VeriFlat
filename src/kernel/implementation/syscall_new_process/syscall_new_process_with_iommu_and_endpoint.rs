...
pub fn syscall_new_process_with_iommu_and_endpoint(
    ...
    ensures
        ...
        ret is Success ==> {
            ...
            &&& exists|source_range: VaRange4K, child_ptr: RwLockProcessPtr, iommu_table_ptr: RwLockPageTableRoot, thread_ptr: RwLockThreadPtr|
                #![trigger kernel_u_new_process_shared(final(steps).steps.spec_index(0).new_u, final(steps).steps.spec_index(source_range.len as int).new_u, parent_ptr, child_ptr, &source_range), final(krnl).it_mp.spec_index(iommu_table_ptr), final(krnl).thr_mp.spec_index(thread_ptr)]
                source_range.wf()
