...
verus! {
    ...
    // module with direct producer postconditions or narrow fold lemmas.
        ...
        pub(super) fn add_new_thread_to_proc_container_and_scheduler(
            ...
            requires
                ...
                old(kernel).cpu_array.spec_index(cpu_id).view()
                    .write_lock_perm_match(&cpu_lock_perm.view()),
                ...
                old(kernel).scheduler_map.spec_index(scheduler_ptr)
                    .write_lock_perm_match(&scheduler_lock_perm.view()),
                ...
                old(kernel).thread_map.spec_index(current_thread_ptr)
                    .write_lock_perm_match(&current_thread_lock_perm.view()),
                ...
                ),
        {
            ...
            proof {
                assert(kernel.process_map.dom().contains(process_ptr)
                ...
                assert(kernel.scheduler_map.dom().contains(scheduler_ptr)
                ...
                assert(kernel.cpu_array.spec_index(cpu_id).view()
                    == old(kernel).cpu_array.spec_index(cpu_id).view()) by {
            ...
            proof {
                assert(new_thread_ptr != current_thread_ptr
            ...
            kernel.wunlock_thread(
                new_thread_ptr, Tracked(&mut *lctx), Tracked(new_thread_lock_perm),
            );
        ...
        pub(crate) fn create_thread_from_staged_page_merged(
            ...
            requires
                ...
                old(kernel).process_map.spec_index(process_ptr)
                    .write_lock_perm_match(&process_lock_perm),
                old(kernel).scheduler_map.dom().contains(scheduler_ptr),
                old(kernel).scheduler_map.spec_index(scheduler_ptr).being_killed() == false,
                typed_lock_map_contains_mode(old(lctx).scheduler_lock_map(), scheduler_ptr, TypedLockMode::Write),
                ...
                old(kernel).scheduler_map.spec_index(scheduler_ptr)
                    .write_lock_perm_match(&scheduler_lock_perm),
                ...
                old(kernel).thread_map.spec_index(staging_thread_ptr)
                    .write_lock_perm_match(&staging_thread_lock_perm),
                ...
                old(kernel).page_array.spec_index(page_ptr2page_index(page_ptr)).view()
                    .write_lock_perm_match(&page_lock_perm),
                old(lctx).page_lock_map().contains_pair(
                    page_ptr2page_index(page_ptr), TypedHeldLock {
                        lock_id: old(kernel).page_array.lock_id_by_index(
            ...
            ensures
                ...
                final(kernel).thread_map.spec_index(page_ptr)
                    .write_lock_perm_match(&ret.1.view()),
                final(kernel).thread_map.dom().contains(page_ptr),
                ...
                final(lctx).thread_lock_map().contains_pair(page_ptr, TypedHeldLock {
                    lock_id: final(kernel).thread_map.lock_id_by_key(page_ptr),
                    mode: TypedLockMode::Write,
                }),
                final(lctx).page_lock_map().contains_pair(
                    page_ptr2page_index(page_ptr), TypedHeldLock {
                ...
                forall|t: RwLockThreadPtr|
                    #![trigger final(kernel).thread_map.spec_index(t).locking_thread()]
                    old(kernel).thread_map.dom().contains(t)
                    ==> final(kernel).thread_map.spec_index(t).locking_thread()
                        == old(kernel).thread_map.spec_index(t).locking_thread(),
                ...
                final(kernel).scheduler_map.dom() == old(kernel).scheduler_map.dom(),
                forall|s: RwLockSchedulerPtr|
                    #![trigger final(kernel).scheduler_map.spec_index(s).locking_thread()]
                    old(kernel).scheduler_map.dom().contains(s)
                    ==> final(kernel).scheduler_map.spec_index(s).locking_thread()
                        == old(kernel).scheduler_map.spec_index(s).locking_thread(),
                ...
                final(kernel).process_map.unchanged_except(
                ...
                final(kernel).page_array.entries_unchanged_except(
                    &old(kernel).page_array, page_ptr2page_index(page_ptr)),
                ...
                final(lctx).thread_lock_map()
                    == old(lctx).thread_lock_map().insert(page_ptr, TypedHeldLock {
                        lock_id: final(kernel).thread_map.lock_id_by_key(page_ptr),
                        mode: TypedLockMode::Write,
                    }),
                ...
                final(lctx).allocator_1g_lock_maps() == old(lctx).allocator_1g_lock_maps(),
        {
            proof {
                ...
                assert(
                    ...
                    && kernel.thread_map.spec_index(staging_thread_ptr)
                        .is_init()
            ...
            let page_mut = kernel.page_array.borrow_mut_typed(
                page_index,
            ...
            {
                let staging_thread_mut = kernel.thread_map
                    .borrow_mut_typed(
                    staging_thread_ptr,
            ...
            proof {
                ...
                add_thread_to_container_sets(
                    &mut kernel.container_map, kernel.root_container,
            ...
            let thread_mut = kernel.thread_map.borrow_mut_typed(
                page_ptr,
                Ghost(lctx.thread_lock_map()),
                Tracked(&*lctx),
                Tracked(&thread_perm),
            ...
            let scheduler_mut = kernel.scheduler_map.borrow_mut_typed(
                scheduler_ptr,
            ...
            proof {
                ...
                assert(kernel.process_management_inv()) by {
                    ...
                    )) by { reveal(process_pcid_allocator_wf); };
    ...
    pub proof fn add_thread_to_container_sets(
        ...
        requires
            ...
            container_uppertree_seq_wf(root_container, *old(container_map)),
            uppers == old(container_map).spec_index(dc).view().uppertree_seq.view(),
    ...
    pub proof fn add_thread_to_ancestor_sets(
        ...
        requires
            old(container_map).perms_wf(),
            forall|c: RwLockContainerPtr|
                #![trigger uppers.contains(c)]
                uppers.contains(c) ==> old(container_map).dom().contains(c),
            uppers.no_duplicates(),
        ensures
            final(container_map).perms_wf(),
            final(container_map).dom() == old(container_map).dom(),
        ...
        decreases uppers.len(),
    {
        if uppers.len() > 0 {
            ...
            assert(uppers.contains(c0)) by {
                uppers.lemma_index_contains(0);
            };
            seq_drop_first_contains_iff_forall(uppers);
    ...
    pub proof fn container_thread_wf_preserved_on_thread_add(
        ...
        requires
            ...
            container_uppertree_seq_wf(root_container, pre_cm),
            uppers == pre_cm.spec_index(dc).view().uppertree_seq.view(),
