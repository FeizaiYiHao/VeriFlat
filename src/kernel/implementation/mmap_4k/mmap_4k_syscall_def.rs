...
/// Every address in the prefix has a concrete L1 table ready for its leaf.
/// This is directory topology only and is therefore absent from `PageTableU`.
pub open spec fn mmap_4k_range_prepared_prefix(
    pagetable: PageTable<PT_TYPE>,
    range: &VaRange4K,
    upper: int,
) -> bool {
...
pub open spec fn mmap_4k_range_prepared(
    pagetable: PageTable<PT_TYPE>,
    range: &VaRange4K,
) -> bool {
    mmap_4k_range_prepared_prefix(pagetable, range, range.len as int)
}
