    ...
    pub(super) fn install_staged_4k_page_table_page(
        ...
        requires
            staged_4k_page_table_op_requires(
                old(self), old(lctx), page_ptr, thread_ptr, pagetable_ptr, indices,
        ...
        proof {
            ...
            assert(self.memory_management_inv()) by {
                ...
                assert(process_pagetable_match(
                    self.process_map, self.pagetable_map,
                )) by {
                    assert({
                        &&& self.process_map == old(self).process_map
                        &&& self.pagetable_map.unchanged_except(
                            &old(self).pagetable_map, pagetable_ptr,
                        )
                        &&& self.pagetable_map.spec_index(pagetable_ptr).view().proc_ptr
                            == old(self).pagetable_map.spec_index(pagetable_ptr).view().proc_ptr
                        &&& self.pagetable_map.spec_index(pagetable_ptr).view().pcid
