...
impl KernelK {
    ...
    pub(super) fn add_new_thread_with_endpoint(
        ...
        requires
            ...
            old(self).endpoint_map.spec_index(endpoint_ptr)
                .locked_by_thread(old(lctx).thread_id()),
            old(self).endpoint_map.spec_index(endpoint_ptr).being_killed() == false,
            ...
            old(self).endpoint_map.lock_id_by_key(endpoint_ptr).major == ENDPOINT_LOCK_MAJOR,
            new_thread_other_objects_unlocked(
            ...
            ),
    {
        ...
        proof {
            ...
            assert(endpoint_objects_unlocked_except(
                ...
                endpoint_objects_unlocked_except_preserved_for_held_unchanged(
                    old(self).endpoint_map, self.endpoint_map, &*lctx,
                    set![endpoint_ptr],
                );
            };
        ...
        proof {
            assert(self.endpoint_map.spec_index(endpoint_ptr).is_init()) by {
        ...
        self.attach_endpoint_reference_and_unlock(
            ...
            Ghost(self.scheduler_map.lock_id_by_key(scheduler_ptr)),
            Ghost(self.process_map.lock_id_by_key(process_ptr)),
            Ghost(self.thread_map.lock_id_by_key(current_thread_ptr)),
        );
        proof {
            assert_sets_equal!(lctx.lock_id_set() == set![
                (self.cpu_array.lock_id_by_index(cpu_id), KernelObjId::Cpu(cpu_id)),
