...
verus! {
        ...
        /// Consume one page staged by `staging_thread_ptr` to create a scheduled
        /// thread in `process_ptr`. The two processes may differ, but both the
        /// staging thread and target process must belong to `container_ptr`.
        pub fn create_thread_from_staged_page_merged(
            ...
            ensures
                ...
                forall|exceptions: Set<RwLockContainerPtr>|
                    #![trigger container_objects_unlocked_except(old(krnl).ctn_mp, old(lctx).thread_id(), exceptions)]
                    container_objects_unlocked_except(old(krnl).ctn_mp, old(lctx).thread_id(), exceptions)
                    ==> container_objects_unlocked_except(final(krnl).ctn_mp, final(lctx).thread_id(), exceptions),
                ...
                forall|exceptions: Set<RwLockProcessPtr>|
                    #![trigger process_objects_unlocked_except(old(krnl).prc_mp, old(lctx).thread_id(), exceptions.insert(process_ptr))]
                    !exceptions.contains(process_ptr)
                    && process_objects_unlocked_except(old(krnl).prc_mp, old(lctx).thread_id(), exceptions.insert(process_ptr))
                    ==> process_objects_unlocked_except(final(krnl).prc_mp, final(lctx).thread_id(), exceptions.insert(process_ptr)),
                ...
                forall|exceptions: Set<PageIndex>|
                    #![trigger page_objects_unlocked_except(old(krnl).pg_arr, old(lctx).thread_id(), exceptions.insert(page_ptr2page_index(page_ptr)))]
                    !exceptions.contains(page_ptr2page_index(page_ptr))
                    && page_objects_unlocked_except(old(krnl).pg_arr, old(lctx).thread_id(), exceptions.insert(page_ptr2page_index(page_ptr)))
    ...
    pub open spec fn new_thread_kernel_transition_framing(
        ...
        &&& post.ctn_mp.spec_index(container_ptr).view_ghost().owned_threads.view() =~= pre.ctn_mp.spec_index(container_ptr).view_ghost().owned_threads.view().insert(page_ptr)
        &&& forall|c: RwLockContainerPtr|
            #![trigger pre.ctn_mp.spec_index(c).view_ghost().owned_threads]
            #![trigger post.ctn_mp.spec_index(c).view_ghost().owned_threads]
            pre.ctn_mp.dom().contains(c) && c != container_ptr ==>
                post.ctn_mp.spec_index(c).view_ghost().owned_threads == pre.ctn_mp.spec_index(c).view_ghost().owned_threads
        &&& forall|c: RwLockContainerPtr|
            #![trigger pre.ctn_mp.spec_index(c).view_ghost().owned_indirect_threads]
            #![trigger post.ctn_mp.spec_index(c).view_ghost().owned_indirect_threads]
            uppers.to_set().contains(c) ==>
                post.ctn_mp.spec_index(c).view_ghost().owned_indirect_threads.view() =~= pre.ctn_mp.spec_index(c).view_ghost().owned_indirect_threads.view().insert(page_ptr)
        &&& forall|c: RwLockContainerPtr|
            #![trigger pre.ctn_mp.spec_index(c).view_ghost().owned_indirect_threads]
            #![trigger post.ctn_mp.spec_index(c).view_ghost().owned_indirect_threads]
            pre.ctn_mp.dom().contains(c) && !uppers.to_set().contains(c) ==>
                post.ctn_mp.spec_index(c).view_ghost().owned_indirect_threads == pre.ctn_mp.spec_index(c).view_ghost().owned_indirect_threads
        &&& forall|c: RwLockContainerPtr|
        ...
        &&& post.prc_mp.spec_index(process_ptr).view_ghost().uppertree_seq == pre.prc_mp.spec_index(process_ptr).view_ghost().uppertree_seq
        &&& post.prc_mp.spec_index(process_ptr).view_ghost().subtree_set == pre.prc_mp.spec_index(process_ptr).view_ghost().subtree_set
        &&& forall|p: RwLockProcessPtr|
            #![trigger pre.prc_mp.spec_index(p)]
            #![trigger post.prc_mp.spec_index(p)]
    ...
    proof fn add_thread_to_container_sets(
        ...
        ensures
            ...
            final(container_map).spec_index(dc).view_ghost().owned_threads.view() =~= old(container_map).spec_index(dc).view_ghost().owned_threads.view().insert(t_ptr),
            forall|c: RwLockContainerPtr|
                #![trigger old(container_map).spec_index(c).view_ghost().owned_threads]
            ...
            forall|c: RwLockContainerPtr|
                #![trigger old(container_map).spec_index(c).view_ghost().owned_indirect_threads]
    ...
    {
        container_map.update_ghost(dc, ContainerGhost {
            owned_threads: Ghost(container_map.spec_index(dc).view_ghost().owned_threads.view().insert(t_ptr)),
            owned_indirect_threads: container_map.spec_index(dc).view_ghost().owned_indirect_threads,
        });
