...
pub(super) fn ipc_enqueue_endpoint_waiter(
    ...
    requires
        ...
        endpoint_lock_perm.view().lock_id()
            == old(endpoint_map).spec_index(endpoint_ptr)
                .locking_thread()->Write_lock_id,
        waiting_state.is_endpoint_waiting(),
    ...
    ensures
        ...
        final(endpoint_map).spec_index(endpoint_ptr).locking_thread()
        ...
        final(endpoint_map).spec_index(endpoint_ptr).view().queue.view()
            == old(endpoint_map).spec_index(endpoint_ptr).view().queue.view()
                .push(thread_ptr),
        final(endpoint_map).spec_index(endpoint_ptr).view().queue.map()
            == old(endpoint_map).spec_index(endpoint_ptr).view().queue.map()
                .insert(node_addr, thread_ptr),
        !old(endpoint_map).spec_index(endpoint_ptr).view().queue.map().dom()
            .contains(node_addr),
            ...
            },
{
    ...
    let endpoint_mut = endpoint_map.borrow_mut(
        endpoint_ptr, Tracked(lctx), endpoint_lock_perm,
    );
    endpoint_mut.enqueue_waiter(
        thread_ptr, waiting_state, node_addr, node_perm,
    );
    proof {
        assert(endpoint_perms_wf(*endpoint_map)) by {
            reveal(endpoint_perms_wf);
            reveal(endpoints_inv);
        };
    }
            ...
            },
{
    ...
    let ret = endpoint_mut.dequeue_waiter(thread_ptr);
    proof {
        assert(endpoint_perms_wf(*endpoint_map)) by {
            reveal(endpoint_perms_wf);
            reveal(endpoints_inv);
...
pub(super) fn ipc_enqueue_scheduled_thread(
    ...
    requires
        ...
        scheduler_lock_perm.view().lock_id()
            == old(scheduler_map).spec_index(scheduler_ptr)
                .locking_thread()->Write_lock_id,
        node_perm.view().is_init(),
        node_perm.view().addr() == node_addr,
        node_perm.view().value().view() == thread_ptr,
    ...
    ensures
        ...
        final(scheduler_map).spec_index(scheduler_ptr).locking_thread()
        ...
        final(scheduler_map).spec_index(scheduler_ptr).view().queue.map()
            == old(scheduler_map).spec_index(scheduler_ptr).view().queue.map()
                .insert(node_addr, thread_ptr),
            ...
            },
{
    ...
    let scheduler_mut = scheduler_map.borrow_mut(
        scheduler_ptr, Tracked(lctx), scheduler_lock_perm,
    );
    scheduler_mut.enqueue_scheduled_thread(
        thread_ptr, node_addr, node_perm,
    );
    proof {
        assert(scheduler_perms_wf(*scheduler_map)) by {
            reveal(scheduler_perms_wf);
        };
    }
}
