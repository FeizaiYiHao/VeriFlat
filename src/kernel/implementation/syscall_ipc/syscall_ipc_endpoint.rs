        ...
        kernel_k_to_kernel_u(*final(kernel)) == kernel_k_to_kernel_u(*old(kernel)),
{
    proof {
        assert({
            &&& kernel.thread_map.view().spec_index(receiver_thread_ptr)
                .is_init()
            &&& kernel.thread_map.view().spec_index(receiver_thread_ptr).addr()
                == receiver_thread_ptr
            &&& kernel.endpoint_map.view().spec_index(payload_endpoint_ptr)
                .is_init()
            &&& kernel.endpoint_map.view().spec_index(payload_endpoint_ptr).addr()
        ...
        assert({
            &&& !kernel.endpoint_map.spec_index(payload_endpoint_ptr).view()
    ...
    proof {
        ...
        assert({
            ...
            &&& kernel.thread_map.lock_id_by_key(receiver_thread_ptr)
                == old(kernel).thread_map.lock_id_by_key(receiver_thread_ptr)
        ...
        lock_id_aligned(final(kernel), final(lctx)),
{
    ...
    proof {
        ...
        assert({
            ...
            &&& kernel.endpoint_map.dom().contains(payload_endpoint_ptr)
    ...
    proof {
        ...
        assert(kernel.process_management_inv()) by {
            ...
            assert({
                ...
                &&& process_thread_wf(
                    kernel.process_map, kernel.thread_map)
                &&& thread_cpu_wf(kernel.thread_map, kernel.cpu_array)
            }) by {
                reveal(container_scheduler_wf);
                reveal(container_thread_wf);
                reveal(process_thread_wf);
