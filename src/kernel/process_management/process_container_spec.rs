...
verus! {
    pub proof fn container_process_wf_proof()
        ensures
            forall|container_perms: LockedMap<RwLockContainerPtr, Container, CONTAINER_HAS_KILL_STATE>, process_perms: LockedMap<RwLockProcessPtr, Process, PROCESS_HAS_KILL_STATE>|
                container_process_wf(container_perms, process_perms) <==> container_process_wf_inner(container_perms, process_perms),
    {}

    pub closed spec fn container_process_wf(container_perms: LockedMap<RwLockContainerPtr, Container, CONTAINER_HAS_KILL_STATE>, process_perms: LockedMap<RwLockProcessPtr, Process, PROCESS_HAS_KILL_STATE>) -> bool{
        container_process_wf_inner(container_perms, process_perms)
    }

    pub open spec fn container_process_wf_inner(container_perms: LockedMap<RwLockContainerPtr, Container, CONTAINER_HAS_KILL_STATE>, process_perms: LockedMap<RwLockProcessPtr, Process, PROCESS_HAS_KILL_STATE>) -> bool{
        &&&
    ...
    pub closed spec fn per_container_process_tree_wf(container_perms: LockedMap<RwLockContainerPtr, Container, CONTAINER_HAS_KILL_STATE>, process_perms: LockedMap<RwLockProcessPtr, Process, PROCESS_HAS_KILL_STATE>) -> bool{
        per_container_process_tree_wf_inner(container_perms, process_perms)
    } 

    pub open spec fn per_container_process_tree_wf_inner(container_perms: LockedMap<RwLockContainerPtr, Container, CONTAINER_HAS_KILL_STATE>, process_perms: LockedMap<RwLockProcessPtr, Process, PROCESS_HAS_KILL_STATE>) -> bool
        recommends
            container_process_wf_inner(container_perms, process_perms),
    {
        &&&
