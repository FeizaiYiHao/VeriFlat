...
verus! {
   pub proof fn process_cpu_wf_proof()
    ...
    pub closed spec fn process_cpu_wf(process_map: LockedMap<RwLockProcessPtr, Process, PROCESS_HAS_KILL_STATE>, cpu_array:LockedArray<Cpu, NUM_CPUS, CPU_HAS_KILL_STATE>) -> bool {
        process_cpu_wf_inner(process_map, cpu_array)
    }
    pub open spec fn process_cpu_wf_inner(process_map: LockedMap<RwLockProcessPtr, Process, PROCESS_HAS_KILL_STATE>, cpu_array:LockedArray<Cpu, NUM_CPUS, CPU_HAS_KILL_STATE>) -> bool {
        &&&
        forall|cpu_i:CpuId|
            ...
            ==>
            {
                ...
                &&&
                cpu_array.spec_index(cpu_i).view().view().current_process is Some && process_map.dom().contains(cpu_array.spec_index(cpu_i).view().view().current_process.unwrap())
                ==>
