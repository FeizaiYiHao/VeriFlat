...
impl PageArray{
    ...
    pub fn page_add_mapping_4k(&mut self, page_index: PageIndex, Tracked(cctx): Tracked<&ConcurrencyContext>, Tracked(lock_perm): Tracked<&LockPerm>, pagetable_root:PageTableRoot, v_addr: VAddr)
        requires
            page_index_wf(page_index),

            old(self).inv(),
            old(self)[page_index]@.wlocked_by(cctx),
        ...
        ensures
            ...
            self[page_index]@.wlocked_by(cctx),
            self[page_index]@.is_init(),
            self[page_index]@@.state == old(self)[page_index]@@.state,
            old(self)[page_index]@@.inv() ==> self[page_index]@@.inv() 
    {
        let mut page = self.phy_pages.take(page_index, Tracked(cctx), Tracked(lock_perm));
        page.ref_count = page.ref_count + 1;
        page.mappings_4k = Ghost(page.mappings_4k@.insert((pagetable_root, v_addr)));
        self.phy_pages.put(page_index, Tracked(cctx),Tracked(lock_perm), page);
    }

    pub fn wlock_page(&mut self, page_index: PageIndex, Tracked(cctx): Tracked<&mut ConcurrencyContext>, lock_id: Ghost<LockId>) -> (ret: Tracked<LockPerm>)
        requires
            page_index_wf(page_index),
            old(self).inv(),
            ...
            wlock_requires(old(self)[page_index]@, old(cctx)),
            old(cctx).lock_id_valid(lock_id@),
        ensures
            self.inv(),
            self.unchanged_except(old(self), page_index),

            wlock_ensures(old(self)[page_index]@, self[page_index]@, lock_id@, cctx.thread_id(), ret@),
            lock_ensures(old(cctx), cctx, lock_id@),
    {
        self.phy_pages.wlock(page_index, Tracked(cctx), lock_id)
    }
